...
from theodolite_mcp.domain.geodesy import (
    WGS84, helmert_transform, geodetic_to_ecef, ecef_to_geodetic, gauss_kruger_forward
)
...
def test_lsa_under_constrained():
    ...
    obs = [ObservationLS(from_pt="P1", to_pt="P2", value=10.0, std_dev=0.01, type="distance")]
    ...
    assert res.standard_deviations == {} # Should not have precision for singular matrix
...
def test_lsa_disconnected_point():
    ...
    obs = [
        ...
        ObservationLS(from_pt="P1", to_pt="P2", value=10.0, std_dev=0.01, type="distance"),
    ]
    # P3 is disconnected
    res = adjust_network_2d(obs, initial)
...
def test_gauss_kruger_on_meridian():
    ...
    # Y (Easting) should be 0 (without false easting)
    assert pytest.approx(y, abs=1e-3) == 0.0
