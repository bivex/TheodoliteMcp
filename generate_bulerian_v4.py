...
"""
Булер'ян «Вогонь» — v4.
Виправлення: DN8 прибрано, show_dn_label=False на всіх допоміжних лінійках,
легенда з поясненнями всіх позначень, димохід з підписами Ø108→Ø150.
"""
...
def _aux(x1, y1, x2, y2, color, dn=15):
    """Допоміжна лінія (рамка, розмірка) — без DN-мітки, без стрілки."""
    return PipeSegment(
...
def build_v4() -> PipelineSchematic:
    pipes = []
    valves = []
    equipment = []
    fittings = []
    ...
    #    Кожна ділянка підписана окремо
    # ================================================================
    cx = bx  # димохід по центру печі
    ...
    # ================================================================
    # 6. МЕБЛІ (з розмірками відступів)
    # ================================================================
    equipment.append(EquipmentSymbol(
    ...
    # Розмірки відступів (без DN-міток!)
    # Піч → ліва стіна
    pipes.append(_aux(0, by, bx-hw-0.5, by, S))
    # Піч → передня стіна
    pipes.append(_aux(bx, 0, bx, by-hh-0.5, S))
    # Безпечна зона → шафа
    pipes.append(_aux(bx+hw+0.5, 2.0, 5.3, 2.0, S))
    ...
    return PipelineSchematic(
        title="Розміщення булер'яна «Вогонь» у хаті 6×7 м (v4)",
        project_number="БЖ-004",
...
def main():
    ...
    with open("bulerian_v4.png", "wb") as f:
        f.write(png)
    print(f"PNG: bulerian_v4.png ({len(png)} bytes)")
    svg = render_pipeline_schematic(plan, "svg")
    with open("bulerian_v4.svg", "wb") as f:
        f.write(svg)
    print(f"SVG: bulerian_v4.svg ({len(svg)} bytes)")
