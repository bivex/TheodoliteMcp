...
def adjust_network_2d(
    ...
    if n_free == 0:
        return LSAResult(
            adjusted_coordinates=initial_coords,
            standard_deviations={},
            unit_weight_variance=1.0,
            iterations=0,
        )
    ...
    for iter_idx in range(max_iters):
        ...
        for i, name in enumerate(free_pts):
            ...
            max_correction = max(
                max_correction, abs(dx_vec[2 * i]), abs(dx_vec[2 * i + 1])
            )

        if max_correction < tolerance:  # Convergence
            # Recompute final residuals using adjusted coordinates
            v_list = []
            for obs in observations:
                if obs.type == "distance":
                    p1, p2 = curr_coords[obs.from_pt], curr_coords[obs.to_pt]
                    dist_calc = math.hypot(p2["x"] - p1["x"], p2["y"] - p1["y"])
                    v_list.append(obs.value - dist_calc)
                elif obs.type == "azimuth":
                    p1, p2 = curr_coords[obs.from_pt], curr_coords[obs.to_pt]
                    dx, dy = p2["x"] - p1["x"], p2["y"] - p1["y"]
                    az_calc = math.degrees(math.atan2(dy, dx))
                    v_list.append(obs.value - az_calc)
                elif obs.type == "angle":
                    if obs.target_pt is None or obs.target_pt not in curr_coords:
                        continue
                    p_v = curr_coords[obs.from_pt]
                    p_1 = curr_coords[obs.to_pt]
                    p_2 = curr_coords[obs.target_pt]
                    dx1, dy1 = p_1["x"] - p_v["x"], p_1["y"] - p_v["y"]
                    dx2, dy2 = p_2["x"] - p_v["x"], p_2["y"] - p_v["y"]
                    az1 = math.degrees(math.atan2(dy1, dx1))
                    az2 = math.degrees(math.atan2(dy2, dx2))
